...
! root is not used. Consider to use the __root value from then
! head.txt to allocate memory in the modrconfig.allocateVariables
! if you need root, because the line below is causing Segmentation
! fault if maxgrds = 1.
! Using maxgrids = 1 root memory (e.g. 420) exceeds nzgmax*nvtyp
! (e.g. 12*30 = 360 )
! Using maxgrids = 4, more memory is allocated doesn't exceeds
! 12*360*4
